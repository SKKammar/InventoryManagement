...
export default function Products() {
  ...
  if (isLoading) return (
    <div className="space-y-8 crt-effect">
      <div className="flex flex-col sm:flex-row justify-between gap-6 border-b-[4px] border-[#ff00ff] pb-4">
        <div className="h-12 w-64 bg-[#112240] animate-pulse border-2 border-[#00ffcc]"></div>
      </div>
      <div className="retro-table-container">
        <table className="w-full retro-table">
          <thead className="bg-[#03040c]">
            <tr>
              <th>SKU</th><th>NAME</th><th>CATEGORY</th><th>PRICE</th><th>QUANTITY</th>
          ...
          <tbody>
            {[...Array(5)].map((_, i) => (
              <tr key={i} className="animate-pulse">
                <td><div className="h-4 bg-[#00ffcc]/20 w-24"></div></td>
                <td><div className="h-4 bg-white/20 w-32"></div></td>
                <td><div className="h-6 bg-[#ff00ff]/20 w-20"></div></td>
                <td><div className="h-4 bg-[#39ff14]/20 w-16"></div></td>
                <td><div className="h-4 bg-[#ffea00]/20 w-12"></div></td>
                {isAdmin && <td><div className="h-8 bg-[#00ffcc]/10 w-20 ml-auto"></div></td>}
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="retro-table-container">
        <table className="w-full retro-table">
          ...
          <tbody>
            ...
            {products.length === 0 && (
              <tr>
                <td colSpan={isAdmin ? "6" : "5"} className="p-16">
                  <div className="flex flex-col items-center justify-center space-y-4">
                    <div className="w-16 h-16 border-4 border-dashed border-[#ffea00] animate-[spin_10s_linear_infinite] flex items-center justify-center text-[#ffea00] font-bold text-2xl">
                       !
                    </div>
                    <div className="text-[#ffea00] font-bold uppercase tracking-widest text-xl">
                      DATABANK EMPTY
                    </div>
                    <p className="text-[#00ffcc] opacity-70 font-mono text-sm max-w-md text-center">
                      No products found in the current inventory logs. Awaiting new data entry...
                    </p>
                  </div>
                </td>
              </tr>
